...
<template>
  <div v-if="item" class="flex flex-col justify-start px-4 sm:px-6 md:px-10 pt-5">
    <div class="mb-6 sm:mb-8">
      ...
      <p class="text-gray-500 text-sm sm:text-base">{{ item.date }}</p>
    </div>

    <p class="text-base sm:text-lg md:text-[14px] leading-6 sm:leading-7" v-html="item.details"></p>

    <div class="flex flex-col sm:flex-row gap-4 mt-10">
      <button @click="router.push(`/edit/${String(route.params.id)}`)" class="w-full sm:w-40 h-10 py-2 bg-[#60CED4] text-white text-sm rounded-md hover:bg-[#50B6BE] transition">
        Edit
      </button>
